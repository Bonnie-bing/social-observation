...
transformed data {
    ...
    matrix[2, 6] initV;
    initV = rep_matrix(0.5,2,6); // 6 stimuli in total, 2 choices per stimulus. (1+0)/2=0.5
}

parameters {
// Declare all parameters as vectors for vectorizing
  // Hyper(group)-parameters
  vector[2] mu_pr; //mean of the parameters,2 paras (learning rate and inverse temperature)
  vector<lower=0>[2] sigma; //variance of the parameters, 2 paras
...
model {
  ...
  for (i in 1:Ns) {
    for (bIdx in 1:Bsubj[i]) {  // new
      ...
      real PE1;       // prediction error, for choice 1
      real PE2;       // prediction error, for choice 2
      ...
      for (t in 1:(Tsubj[i, bIdx])) {  // new
        ...
        // prediction error
        PE1   =  actions[1,choice[i, bIdx, t]] - ev[1,cond[i,bIdx,t]];  //new
        PE2   =  actions[2,choice[i, bIdx, t]] - ev[2,cond[i,bIdx,t]];  //new

        // value updating (learning)
        ev[1,cond[i,bIdx,t]]   += eta[i] * PE1;   //new
        ev[2,cond[i,bIdx,t]]   += eta[i] * PE2;   //new
