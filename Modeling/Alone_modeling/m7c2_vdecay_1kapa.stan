...
transformed parameters {
  ...
  real<lower=0, upper=5> kapa[Ns,3];
  ...
  for (i in 1:Ns) {
    ...
    kapa[i,1]    = Phi_approx(mu_pr[3] + sigma[3] * kapa_pr1[i])*5;
	  kapa[i,2]    = Phi_approx(mu_pr[4] + sigma[4] * kapa_pr2[i])*5;
	  kapa[i,3]    = Phi_approx(mu_pr[5] + sigma[5] * kapa_pr3[i])*5;
...
generated quantities {
  ...
  real<lower=0, upper=5> mu_kapa[3];
  ...
  mu_kapa[1]     = Phi_approx(mu_pr[3])*5;
  mu_kapa[2]     = Phi_approx(mu_pr[4])*5;
  mu_kapa[3]     = Phi_approx(mu_pr[5])*5;
