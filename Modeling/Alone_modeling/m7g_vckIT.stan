...
// Declare all parameters as vectors for vectorizing
  ...
  vector[Ns] tauck_pr1;    // tau: Inverse temperature for ck
  vector[Ns] tauck_pr2;    // tau: Inverse temperature for ck
  vector[Ns] tauck_pr3;    // tau: Inverse temperature for ck
...
transformed parameters {
  ...
  real<lower=0, upper=10> tauck[Ns,3];
  ...
  for (i in 1:Ns) {
    ...
    tauck[i,1]     = Phi_approx(mu_pr[5] + sigma[5] * tauck_pr1[i]) * 10;
    tauck[i,2]     = Phi_approx(mu_pr[6] + sigma[6] * tauck_pr2[i]) * 10;
    tauck[i,3]     = Phi_approx(mu_pr[7] + sigma[7] * tauck_pr3[i]) * 10;
...
model {
  ...
  tauck_pr1   ~ normal(0, 1.0);
  tauck_pr2   ~ normal(0, 1.0);
  tauck_pr3   ~ normal(0, 1.0);
  ...
  for (i in 1:Ns) {
    for (bIdx in 1:Bsubj[i]) {  // new
      ...
      for (t in 1:(Tsubj[i, bIdx])) {  // new
        ...
        prob[1] = 1 / (1 + exp(tau[i] * (ev[2,cond[i,bIdx,t]] - ev[1,cond[i,bIdx,t]]) + tauck[i,probab[i,bIdx,t]] * (evck[2,cond[i,bIdx,t]] - evck[1,cond[i,bIdx,t]]))); // according to softmax, MUST BE U2-U1 here!!!!
        prob_1_ = prob[1];
        prob[2] = 1 - prob_1_;
...
generated quantities {
  ...
  real<lower=0, upper=10> mu_tauck[3];
  ...
  mu_tauck[1]    = Phi_approx(mu_pr[5]) * 10;
  mu_tauck[2]    = Phi_approx(mu_pr[6]) * 10;
  mu_tauck[3]    = Phi_approx(mu_pr[7]) * 10;
