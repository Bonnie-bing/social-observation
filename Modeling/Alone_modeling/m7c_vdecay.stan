...
// Declare all parameters as vectors for vectorizing
  ...
  vector[12] mu_pr; //mean of the parameters,6 paras (2 learning rates and inverse temperature) + (1 LR and it for CK) + forget para
  vector<lower=0>[12] sigma; //variance of the parameters, 2 paras

  // Subject-level raw parameters (for Matt trick)
  vector[Ns] etar_pr;  // etar: learning rate for reward
  vector[Ns] etap_pr;  // etap: learning rate for punishment
  
  vector[Ns] kapar_pr1;  // kapa: decaying parameter(>=0)
  vector[Ns] kapar_pr2;  // kapa: decaying parameter(>=0)
  vector[Ns] kapar_pr3;  // kapa: decaying parameter(>=0)
  
  
  vector[Ns] kapap_pr1;  // kapa: decaying parameter(>=0)
  vector[Ns] kapap_pr2;  // kapa: decaying parameter(>=0)
  vector[Ns] kapap_pr3;  // kapa: decaying parameter(>=0)
...
transformed parameters {
  ...
  real<lower=0> kapar[Ns,3];
  real<lower=0> kapap[Ns,3];
  ...
  for (i in 1:Ns) {
    ...
    kapar[i,1]    = exp(mu_pr[3] + sigma[3] * kapar_pr1[i]);
	  kapar[i,2]    = exp(mu_pr[4] + sigma[4] * kapar_pr2[i]);
	  kapar[i,3]    = exp(mu_pr[5] + sigma[5] * kapar_pr3[i]);
	  
	  kapap[i,1]    = exp(mu_pr[6] + sigma[6] * kapap_pr1[i]);
	  kapap[i,2]    = exp(mu_pr[7] + sigma[7] * kapap_pr2[i]);
	  kapap[i,3]    = exp(mu_pr[8] + sigma[8] * kapap_pr3[i]);
    
    tau[i]       = Phi_approx(mu_pr[9] + sigma[9] * tau_pr[i]) * 10;
	
	  etack[i]     = Phi_approx(mu_pr[10] + sigma[10] * etack_pr[i]);
    tauck[i]     = Phi_approx(mu_pr[11] + sigma[11] * tauck_pr[i]) * 10;
    
    fp[i]        = Phi_approx(mu_pr[12] + sigma[12] * fp_pr[i]);
  }
...
model {
  ...
  kapar_pr1  ~ normal(0, 1.0);
  kapar_pr2  ~ normal(0, 1.0);
  kapar_pr3  ~ normal(0, 1.0);
  
  kapap_pr1  ~ normal(0, 1.0);
  kapap_pr2  ~ normal(0, 1.0);
  kapap_pr3  ~ normal(0, 1.0);
    ...
    for (bIdx in 1:Bsubj[i]) {  // new
      ...
      for (t in 1:(Tsubj[i, bIdx])) {  // new
        ...
        if (outcome[i, bIdx, t] > 0){ // a pos outcome
          ev[choice[i, bIdx, t],cond[i,bIdx,t]]   += etar[i] *(exp(-kapar[i,probab[i,bIdx,t]]*(tn_perC[i,bIdx,t]-1)/10))* PE;   //new
          } else { // a neg outcome
          ev[choice[i, bIdx, t],cond[i,bIdx,t]]   += etap[i] *(exp(-kapap[i,probab[i,bIdx,t]]*(tn_perC[i,bIdx,t]-1)/10))* PE;   //new
        }
...
generated quantities {
  ...
  real<lower=0> mu_kapar[3];
  real<lower=0> mu_kapap[3];
  ...
  mu_kapar[1]     = exp(mu_pr[3]);
  mu_kapar[2]     = exp(mu_pr[4]);
  mu_kapar[3]     = exp(mu_pr[5]);
  
  mu_kapap[1]     = exp(mu_pr[6]);
  mu_kapap[2]     = exp(mu_pr[7]);
  mu_kapap[3]     = exp(mu_pr[8]);
  
  mu_tau      = Phi_approx(mu_pr[9]) * 10;
  
  mu_etack    = Phi_approx(mu_pr[10]);
  mu_tauck    = Phi_approx(mu_pr[11]) * 10;
  
  mu_fp       = Phi_approx(mu_pr[12]);
