function data = sim_fit_m7c2(param,design,subn,sampn)
...
data   = zeros(length(design),18);% 1=subid,2=block,3=cond,4=feedback,5=choice,6=trial_perC2,7=probab,8=delay,9=ACC, 10=choice_sim,11=choice_prob_sim,12=PE,13=PE1,14=PE2,15=ev1,16=ev2,17=evck1,18=evck2,19=feed_sim
...
for nb = 1:blocks %loop over each block
    ...
    for nt=1:length(bData) % loop over each trial of the block
        ...
        %% determine the feedback of the simulated choice
        if c(t)==choice(nt)
            feed(t) = feedback(nt);
        else
            feed(t) = -feedback(nt);
        ...
        PE(t)       = feed(t) - ev(c(t),cond(nt));    % outcome - ev; here we use the simulated feeback and choice
        PE1(t)      = actions(1,c(t)) - evck(1,cond(nt)); % actions - ck; here we use the simulated choice
        PE2(t)      = actions(2,c(t)) - evck(2,cond(nt)); % actions - ck; here we use the simulated choice
        ...
        if feed(t) >0 % a pos outcome,here we use the simulated feeback
            ev(c(t),cond(nt))      = ev(c(t),cond(nt)) + etar*(exp(-kapa(probab(nt))*(trial_perC(nt)-1)/10))*PE(t); % here we use the simulated choice
        else % a neg outcome
            ev(c(t),cond(nt))      = ev(c(t),cond(nt)) + etap*(exp(-kapa(probab(nt))*(trial_perC(nt)-1)/10))*PE(t); % here we use the simulated choice
        end
...
% 1=subid,2=block,3=cond,4=feedback,5=choice,6=trial_perC2,7=probab,8=delay,9=ACC, 10=choice_sim,11=choice_prob_sim,12=PE,13=PE1,14=PE2,15=ev1,16=ev2,17=evck1,18=evck2,19=feed_sim
