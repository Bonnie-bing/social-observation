...
// Declare all parameters as vectors for vectorizing
  ...
  vector[10] mu_pr; //mean of the parameters,6 paras (2 learning rates and inverse temperature) + (1 LR and it for CK) + forget para
  vector<lower=0>[10] sigma; //variance of the parameters, 2 paras

  // Subject-level raw parameters (for Matt trick)
  vector[Ns] etar_pr1;  // etar: learning rate for reward
  vector[Ns] etar_pr2;  // etar: learning rate for reward
  vector[Ns] etar_pr3;  // etar: learning rate for reward
  
  vector[Ns] etap_pr1;  // etar: learning rate for punishment
  vector[Ns] etap_pr2;  // etar: learning rate for punishment
  vector[Ns] etap_pr3;  // etar: learning rate for punishment
...
transformed parameters {
  ...
  real<lower=0, upper=1> etar[Ns,3];
  real<lower=0, upper=1> etap[Ns,3];
  ...
  for (i in 1:Ns) {
    etar[i,1]      = Phi_approx(mu_pr[1] + sigma[1] * etar_pr1[i]);
    etar[i,2]      = Phi_approx(mu_pr[2] + sigma[2] * etar_pr2[i]);
    etar[i,3]      = Phi_approx(mu_pr[3] + sigma[3] * etar_pr3[i]);
    
    etap[i,1]      = Phi_approx(mu_pr[4] + sigma[4] * etap_pr1[i]);
    etap[i,2]      = Phi_approx(mu_pr[5] + sigma[5] * etap_pr2[i]);
    etap[i,3]      = Phi_approx(mu_pr[6] + sigma[6] * etap_pr3[i]);
    
    tau[i]       = Phi_approx(mu_pr[7] + sigma[7] * tau_pr[i]) * 10;
	
	  etack[i]     = Phi_approx(mu_pr[8] + sigma[8] * etack_pr[i]);
    tauck[i]     = Phi_approx(mu_pr[9] + sigma[9] * tauck_pr[i]) * 10;
    
    fp[i]        = Phi_approx(mu_pr[10] + sigma[10] * fp_pr[i]);
  }
...
model {
  ...
  etar_pr1  ~ normal(0, 1.0);
  etar_pr2  ~ normal(0, 1.0);
  etar_pr3  ~ normal(0, 1.0);
  
  etap_pr1  ~ normal(0, 1.0);
  etap_pr2  ~ normal(0, 1.0);
  etap_pr3  ~ normal(0, 1.0);
    ...
    for (bIdx in 1:Bsubj[i]) {  // new
      ...
      for (t in 1:(Tsubj[i, bIdx])) {  // new
        ...
        if (outcome[i, bIdx, t] > 0){ // a pos outcome
          ev[choice[i, bIdx, t],cond[i,bIdx,t]]   += etar[i,probab[i,bIdx,t]] * PE;   //new
          } else { // a neg outcome
          ev[choice[i, bIdx, t],cond[i,bIdx,t]]   += etap[i,probab[i,bIdx,t]] * PE;   //new
        }
...
generated quantities {
  ...
  real<lower=0, upper=1> mu_etar[3];
  real<lower=0, upper=1> mu_etap[3];
  ...
  mu_etar[1]     = Phi_approx(mu_pr[1]); // must define mu_eta here
  mu_etar[2]     = Phi_approx(mu_pr[2]); // must define mu_eta here
  mu_etar[3]     = Phi_approx(mu_pr[3]); // must define mu_eta here
  
  
  mu_etap[1]     = Phi_approx(mu_pr[4]); // must define mu_eta here
  mu_etap[2]     = Phi_approx(mu_pr[5]); // must define mu_eta here
  mu_etap[3]     = Phi_approx(mu_pr[6]); // must define mu_eta here
  
  mu_tau      = Phi_approx(mu_pr[7]) * 10;
  
  mu_etack    = Phi_approx(mu_pr[8]);
  mu_tauck    = Phi_approx(mu_pr[9]) * 10;
  
  mu_fp       = Phi_approx(mu_pr[10]);
