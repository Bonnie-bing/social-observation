...
// Declare all parameters as vectors for vectorizing
  ...
  vector[Ns] etack_pr1;  // etar: learning rate for ck
  vector[Ns] etack_pr2;  // etar: learning rate for ck
  vector[Ns] etack_pr3;  // etar: learning rate for ck
...
transformed parameters {
  ...
  real<lower=0, upper=1> etack[Ns,3];
  ...
  for (i in 1:Ns) {
    ...
    tau[i]       = Phi_approx(mu_pr[3] + sigma[3] * tau_pr[i]) * 10;
	  ...
	  etack[i,1]     = Phi_approx(mu_pr[4] + sigma[4] * etack_pr1[i]);
	  etack[i,2]     = Phi_approx(mu_pr[5] + sigma[5] * etack_pr2[i]);
	  etack[i,3]     = Phi_approx(mu_pr[6] + sigma[6] * etack_pr3[i]);
...
model {
  ...
  etack_pr1  ~ normal(0, 1.0);
  etack_pr2  ~ normal(0, 1.0);
  etack_pr3  ~ normal(0, 1.0);
      ...
      for (t in 1:(Tsubj[i, bIdx])) {  // new
        ...
        if (outcome[i, bIdx, t] > 0){ // a pos outcome
          ...
          ev[choice[i, bIdx, t],cond[i,bIdx,t]]   += etap[i] * PE;   //new
		    ...
		    evck[1,cond[i,bIdx,t]]   += etack[i,probab[i,bIdx,t]] * PE1;   //new
        evck[2,cond[i,bIdx,t]]   += etack[i,probab[i,bIdx,t]] * PE2;   //new
...
generated quantities {
  ...
  real<lower=0, upper=1> mu_etack[3];
  ...
  mu_etack[1]    = Phi_approx(mu_pr[4]);
  mu_etack[2]    = Phi_approx(mu_pr[5]);
  mu_etack[3]    = Phi_approx(mu_pr[6]);
