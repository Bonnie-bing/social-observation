...
// Declare all parameters as vectors for vectorizing
  ...
  vector[Ns] tau_pr1;    // tau: Inverse temperature
  vector[Ns] tau_pr2;    // tau: Inverse temperature
  vector[Ns] tau_pr3;    // tau: Inverse temperature
...
transformed parameters {
  ...
  real<lower=0, upper=10> tau[Ns,3];
  ...
  for (i in 1:Ns) {
    ...
    tau[i,1]       = Phi_approx(mu_pr[3] + sigma[3] * tau_pr1[i]) * 10;
    tau[i,2]       = Phi_approx(mu_pr[4] + sigma[4] * tau_pr2[i]) * 10;
    tau[i,3]       = Phi_approx(mu_pr[5] + sigma[5] * tau_pr3[i]) * 10;
	
	  etack[i]     = Phi_approx(mu_pr[6] + sigma[6] * etack_pr[i]);
    tauck[i]     = Phi_approx(mu_pr[7] + sigma[7] * tauck_pr[i]) * 10;
    
    fp[i]        = Phi_approx(mu_pr[8] + sigma[8] * fp_pr[i]);
  }
...
model {
  ...
  tau_pr1   ~ normal(0, 1.0);
  tau_pr2   ~ normal(0, 1.0);
  tau_pr3   ~ normal(0, 1.0);
  ...
  for (i in 1:Ns) {
    for (bIdx in 1:Bsubj[i]) {  // new
      ...
      for (t in 1:(Tsubj[i, bIdx])) {  // new
        ...
        prob[1] = 1 / (1 + exp(tau[i,probab[i,bIdx,t]] * (ev[2,cond[i,bIdx,t]] - ev[1,cond[i,bIdx,t]]) + tauck[i] * (evck[2,cond[i,bIdx,t]] - evck[1,cond[i,bIdx,t]]))); // according to softmax, MUST BE U2-U1 here!!!!
        prob_1_ = prob[1];
        prob[2] = 1 - prob_1_;
...
generated quantities {
  ...
  real<lower=0, upper=10> mu_tau[3];
  ...
  mu_tau[1]   = Phi_approx(mu_pr[3]) * 10;
  mu_tau[2]   = Phi_approx(mu_pr[4]) * 10;
  mu_tau[3]   = Phi_approx(mu_pr[5]) * 10;
  
  mu_etack    = Phi_approx(mu_pr[6]);
  mu_tauck    = Phi_approx(mu_pr[7]) * 10;
  
  mu_fp       = Phi_approx(mu_pr[8]);
