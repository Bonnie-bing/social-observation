//The RW + forgetting model, RW values decay with delays
...
data {
  ...
  int<lower=0, upper=6> cond[Ns, Bs, Ts];  // condition for each subject-block-trial, 0 for allowing for missing values
...
// Declare all parameters as vectors for vectorizing
  ...
  vector[3] mu_pr; //mean of the parameters,3 paras (learning rate, forgeting param and inverse temperature)
  vector<lower=0>[3] sigma; //variance of the parameters, 3 paras

  // Subject-level raw parameters (for Matt trick)
  vector[Ns] eta_pr;  // eta: learning rate
...
transformed parameters {
  ...
  vector<lower=0, upper=1>[Ns] fp;
  vector<lower=0, upper=20>[Ns] tau;

  for (i in 1:Ns) {
    eta[i]       = Phi_approx(mu_pr[1] + sigma[1] * eta_pr[i]);
    fp[i]        = Phi_approx(mu_pr[2] + sigma[2] * fp_pr[i]);
    tau[i]       = Phi_approx(mu_pr[3] + sigma[3] * tau_pr[i]) * 20;
  }
  ...
  for (i in 1:Ns) {
    for (bIdx in 1:Bsubj[i]) {  // new
      ...
      for (t in 1:(Tsubj[i, bIdx])) {  // new
        ...
        // value updating (learning) for RW
         ev[choice[i, bIdx, t],cond[i,bIdx,t]]   += eta[i] * PE;   //new
...
generated quantities {
  ...
  mu_eta    = Phi_approx(mu_pr[1]); // must define mu_eta here
  mu_fp     = Phi_approx(mu_pr[2]);
